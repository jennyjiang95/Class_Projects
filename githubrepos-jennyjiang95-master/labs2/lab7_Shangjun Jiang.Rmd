...
rbinom (20,1,0.9)
  #rbinom(1,1,p)
  #flip <- sample (0:1, rep=T, prob= c(1-p, p))

FlipTilHeads <- function (p){
  n <- 0
  flip <- 0
 while (flip == 0) {
  flip <- rbinom (1,1,p)
...
```{r}
# your code
CFracToDec <- function (v){
  i <- length(v)
  outcome <- v[i]
  for (i in (length(v)-1):1) {
    outcome <- v[i]+1/outcome
    i <- i-1
  }
  return (outcome)
}
...
```{r, eval = F}
Hailstone(1)
# sequence = 1; stop_time = 0
Hailstone(5) 
# sequence = (5, 16, 8, 4, 2, 1); stop_time = 5
Hailstone(27) 
# sequence = too long for this comment; stop_time = 111
```
