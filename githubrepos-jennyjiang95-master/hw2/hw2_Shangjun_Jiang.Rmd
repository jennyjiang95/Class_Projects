...
```{r}
# Problem 1
files_generator <- function (x){
  print (str_c("plot", str_pad ((1:100), 3, "left", "0"), ".png"))
}
files100 <- files_generator(100)
...
# is_hex()
is_hex <- function (x){
str_detect(x, "#[0-9aAbBcCdDeEfF]{6}$")
}
```
...
paste (rev (str_split(x, "")[[1]]), collapse = "")
}
...
remove_tag <- function (x){
front <-str_replace_all(x, "<[a-z]*>", "")
back <- str_replace_all(front, "</[a-z]*>", "")
return (back)
}
